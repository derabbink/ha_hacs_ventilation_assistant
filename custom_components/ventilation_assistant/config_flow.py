...
class VentilationAssistantConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_global(
        ...
        """Create or replace the singleton global defaults entry."""
        ...
        if user_input is not None:
            return self.async_create_entry(
                title="Ventilation Assistant defaults",
...
class VentilationAssistantOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        kind = self.config_entry.data[CONF_KIND]
        if user_input is not None:
            options = (
        ...
        return self.async_show_form(
            step_id="init",
            data_schema=_device_schema(
                {**self.config_entry.options, CONF_NAME: self.config_entry.data[CONF_NAME]}
            ),
        )
    ...
    return vol.Schema(
        {
            ...
            vol.Required(
                CONF_PRIORITY,
                default=defaults.get(CONF_PRIORITY, Priority.TEMPERATURE),
